...
export const calculateEmiOptions = async ({
  ...
  const { rows } = await pool.query(
    ...
    WHERE LOWER(r.vehicle_type) = LOWER($1)
  ...
  return rows.map((bank) => {
    const minRate = Number(bank.min_interest_rate);
    const maxRate = Number(bank.max_interest_rate);
    const interest = (minRate + maxRate) / 2;

    const monthlyRate = interest / 12 / 100;
    const months = tenure_years * 12;

    // Handle zero-interest edge case gracefully
